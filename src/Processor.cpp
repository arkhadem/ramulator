...
Trace::Trace(const char* trace_fname)
    : file(trace_fname)
...
bool Trace::get_timedtrace_request(long& req_addr, Request::Type& req_type, long& req_clock)
{
    string line;
    size_t pos;
    ...
    std::string req_opcode = line.substr(0, pos);
    if (req_opcode.compare("read") == 0)
        req_type = Request::Type::READ;
    else if (req_opcode.compare("write") == 0)
        req_type = Request::Type::WRITE;
    else if (req_opcode.compare("compute") == 0)
