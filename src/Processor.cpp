        ...
        while ((inserted < window.ipc) && (window.is_full() == false)) {
            ...
            if (bubble_cnt > 0) {
                ...
                if ((req_opcode.find("load") != string::npos) || (req_opcode.find("store") != string::npos)) {
                    ...
                    request = Request(req_opcode, req_en, req_addr, addr_end, SA_id, SA_id_dst, callback, id, Request::UnitID::CORE);
                    window.insert(false, request);
                } else {
                    // it's a computational GPIC instruction
                    request = Request(req_opcode, req_en, -1, -1, SA_id, SA_id_dst, callback, id, Request::UnitID::CORE);
                    window.insert(true, request);
                }
...
void Window::set_ready(Request req)
{
    ...
    for (int i = 0; i < load; i++) {
        ...
        if ((sent_list.at(index) == true) && (ready_list.at(index) == false) && (req_list.at(index) == req)) {
