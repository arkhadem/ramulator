...
bool Cache::send(Request req)
{
    if (req.type == Request::Type::GPIC) {
        ...
        if (gpic_instruction_queue[req.SA_id].size() >= MAX_GPIC_QUEUE_SIZE) {
            return false;
        } else {
            if (req.opcode.find("move") != string::npos) {
                if (gpic_instruction_queue[req.SA_id_dst].size() >= MAX_GPIC_QUEUE_SIZE) {
                    return false;
                } else {
                    hint("%s set for start in %d clock cycles\n", req.c_str(), latency_each[int(level)]);
                    gpic_instruction_queue[req.SA_id].push_back(make_pair(cachesys->clk + latency_each[int(level)], req));
                    gpic_instruction_queue[req.SA_id_dst].push_back(make_pair(cachesys->clk + latency_each[int(level)], req));
                    return true;
                }
            } else {
                hint("%s set for start in %d clock cycles\n", req.c_str(), latency_each[int(level)]);
